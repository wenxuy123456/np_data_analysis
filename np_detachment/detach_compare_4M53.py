...
# Group fig5 files by their condition suffix (hh, hl, lh, etc.) so files
# that share a condition (e.g. multiple "hl" files across different
# prefixes/runs) get combined into a single dataset before plotting.
...
def process_condition_group(files, condition):
    ...
    output_file_fig5 = os.path.join(fig5_dir, f"np_detachment_fig5_4M5.3_{condition}_combined_sorted.xlsx")
...
def main():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(script_dir, "outputs", "4M5.3_all_conditions.png"), dpi=150, bbox_inches="tight")
